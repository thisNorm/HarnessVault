...
| 축 | 세는 것 | 기본값 | 걸리면 |
| --- | --- | --- | --- |
| 계정 | 같은 이메일의 실패 **횟수** | 10회 / 15분 | 비밀번호 확인도 하지 않고 429 |
| IP | 한 IP가 실패시킨 **서로 다른 계정 수** | 20개 / 15분 | 틀린 자격증명만 429 |

- **존재하지 않는 계정도 똑같이 잠긴다** — 잠기는지 여부로 계정 존재가 새어 나가지 않게 하기 위해서다.
- **IP 축은 맞는 비밀번호를 막지 않는다.** 막으면 NAT 뒤 사무실 하나가 통째로 로그인하지 못하게 되고, 그 피해가 막으려던 공격보다 크다.
- IP 축이 횟수가 아니라 계정 수를 세는 이유도 같다. 횟수로 세면 20명의 정상 오타와 스프레이 공격이 같은 숫자로 보인다.
- 프록시 뒤에 있다면 `TRUST_PROXY=true`로 켜야 실제 클라이언트 IP를 본다. **기본은 꺼져 있다** — `X-Forwarded-For`는 위조 가능한 헤더라, 프록시가 없는데 믿으면 요청마다 다른 IP를 적어 제한을 무력화할 수 있다.
