...
| 축 | 세는 것 | 기본값 |
| --- | --- | --- |
| 계정 | 같은 이메일의 실패 **횟수** | `LOGIN_MAX_ATTEMPTS` 10회 / 15분 |
| IP | 한 IP가 실패시킨 **서로 다른 계정 수** | `LOGIN_MAX_ACCOUNTS_PER_IP` 20개 / 15분 |
| 유예 | 스프레이 IP에서도 (IP, 계정)마다 확인해 주는 횟수 | `LOGIN_VERIFY_GRACE_PER_PAIR` 3회 |
